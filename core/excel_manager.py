...
class TranslationEntry:
    ...
    def from_dict(cls, data: dict) -> 'TranslationEntry':
        """딕셔너리로부터 생성"""
        # RPG Maker JSON 형식 vs Excel 내보내기 형식 구분
        if 'original' in data and 'translated' in data:
            # RPG Maker JSON 형식 (영어 키)
            entry = cls(
                file_path=data.get('file', ''),
                line_number=0,  # RPG Maker는 줄번호 없음
                japanese=data.get('original', ''),
    ...
    def export_to_excel(
        ...
        for i, e in enumerate(entries):
            if isinstance(e, dict):
                # RPG Maker / Unity 게임용 dict
                # RPG Maker는 'original'/'translated', Unity는 동일한 키 사용
                data.append({
                    'ID': e.get('id', i + 1),  # RPG Maker ID 사용
                    '파일명': e.get('file', 'unknown'),
                    '줄번호': '',  # Unity/RPG Maker는 줄번호 없음
                    '원문': e.get('original', ''),
                    'AI 번역': e.get('translated', ''),
                    '수정본': ''
                })
            elif isinstance(e, TranslationEntry):
                # TranslationEntry 객체는 to_dict() 사용
                data.append(e.to_dict())
            else:
                # 알 수 없는 타입
                print(f"⚠️ 알 수 없는 엔트리 타입: {type(e)}")
                continue
